...
export default class EditorPage extends PureComponent {
	...
	handleFan = async () => {
		...
		if (this.state.iAmAFan) {
			let id = this.props.data.oneComponent.fans.find(
				fan => fan.user_id === this.props.authenticatedId
			).id;
											...
											<div
												...
												onClick={() =>
													this.props.history.push(
														`/users/${this.props.data.oneComponent.owner_user_id}`
													)}
											/>
